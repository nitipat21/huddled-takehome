...
function seedUsers(amount: number): void {
  ...
  `);
	...
	const users: User[] = Array.from({ length: amount }, (_, i) => ({
		username: `User ${i + 1}`,
		timezone: availableTimezones[Math.floor(Math.random() * availableTimezones.length)]
	}));
...
function seedUserSessions(amount: number): void {
  ...
  `);
	...
	// Get all user ids
	const userIdsQuery = db.prepare<{ id: number }, []>('SELECT id FROM users');
...
function seedArtists(amount: number): void {
  ...
  `);
	...
	const query: Statement = db.prepare('INSERT INTO artists (name) VALUES ($name)');
...
function seedTracks(amount: number): void {
  ...
  `);
	...
	const query: Statement = db.prepare('INSERT INTO tracks (title, artist_id) VALUES (?, ?)');
...
function seedVisits(amount: number): void {
  ...
  `);
	...
	// Get all artist ids
	const artistIdsQuery = db.prepare<{ id: number }, []>('SELECT id FROM artists');
	const artistIds = artistIdsQuery.all();

	// Get all sessions
	const sessionsQuery = db.prepare<{ id: number }, []>('SELECT id FROM sessions');
...
function seedUserEvents(amount: number): void {
  ...
  `);
	...
	// Get all tracks
	const tracksQuery = db.prepare<{ id: number }, []>('SELECT id FROM tracks');
	...
	db.transaction(() => {
		userEvents.forEach(
			({ user_id, artist_id, event_type, event_target, created_at }: UserEvent) => {
