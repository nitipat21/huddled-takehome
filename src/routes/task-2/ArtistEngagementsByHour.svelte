<script lang="ts">
	...
	type ArtistEngagementsByHourProps = {
		data: HeatmapData[];
	};

	const { data }: ArtistEngagementsByHourProps = $props();

	onMount(() => {
		const container = d3.select('#heatmap-by-hour-container');
		...
		// Tooltip
		const tooltip = d3
			.select('#heatmap-by-hour-container')
			.append('div')
		...
		g.selectAll('.heatmap-cell')
			...
			.on('mouseover', function (_, d) {
				...
				tooltip.html(
					`${artistIdToName.get(d.artist_id)}<br>Hour: ${d.hour}:00<br><b>Engagement: ${d.total_engagements}</b>`
				);
			})
		...
		g.append('g')
			...
			.call(d3.axisBottom(xScale).tickFormat((d) => formatHour(+d)))
			.selectAll('text')
...
<div id="heatmap-by-hour-container" class="mx-auto w-full max-w-[1440px] rounded-lg bg-white p-8">
	<h1 class="text-center text-2xl font-bold">Artist Engagement by Hour of Day</h1>
</div>
